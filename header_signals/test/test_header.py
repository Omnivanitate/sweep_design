...
import header.defaults.names as dn
from header.relation_header import NamedRelation

class TestHeaderRelation(unittest.TestCase):

    def check_equal(self, r_named: NamedRelation, name, category):
    ...
    def setUp(self) -> None:
        ...
        self.nr = NamedRelation(self.name, self.category)
        self.dname = 'R_1'
        self.dcategory = 'relation'
        self.dnr = NamedRelation()

    def tearDown(self) -> None:
        NamedRelation._quantity = 1
    ...
    def test_select_data(self):
        ...
        self.check_equal(nr, dn.make_default_name_select_data(self.nr, x_start, x_end), self.dcategory)
    ...
    def test_diff(self):
        nr = self.nr.diff()
        self.check_equal(nr, dn.make_default_diff_name(self.nr), 
    ...
    def test_integrate(self):
        ...
        self.check_equal(nr, dn.make_default_integrate_name(self.nr),
    ...
    def test_interpolate_extrapolate(self):
        ...
        self.check_equal(nr, dn.make_default_interpolate_name(self.nr, new_x), self.nr.category)
    ...
    def test_shift(self):
        ...
        self.check_equal(nr, dn.make_default_shift_name(self.nr, x_shift), self.nr.category)
    ...
    def test_equalize(self):
        new_x = np.array([0., 1.])
        nr, nr2 = NamedRelation.equalize(self.nr, self.nr, new_x)
        
        self.check_equal(nr, dn.make_default_interpolate_name(self.nr, new_x), self.nr.category)
        self.check_equal(nr2, dn.make_default_interpolate_name(self.nr, new_x), self.nr.category)

        nr3, nr4 = NamedRelation.equalize(self.nr, self.nr, new_x, 
                        self.name, self.name, self.category, self.category)
        self.check_equal(nr3, self.name, self.category)
        self.check_equal(nr4, self.name, self.category)
    
    def test_correlate(self):
        nr = NamedRelation.correlate(self.nr, self.nr)
        self.check_equal(nr, dn.make_default_name_correlation(self.nr, self.nr), self.nr.category)
        nr2 = NamedRelation.correlate(self.nr, self.nr, self.name, self.category)
        self.check_equal(nr2, self.name, self.category)

    def test_correlate(self):
        nr = NamedRelation.convolve(self.nr, self.nr)
        self.check_equal(nr, dn.make_default_name_convolution(
            self.nr, self.nr), self.nr.category)
        nr2 = NamedRelation.convolve(
            self.nr, self.nr, self.name, self.category)
        self.check_equal(nr2, self.name, self.category)

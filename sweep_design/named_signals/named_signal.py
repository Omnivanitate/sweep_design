...
class NamedSpectrum(NamedRelation):
    ...
    def __init__(self, f: Any, s_a: Any = None, name: InName = None, 
        ...
        self.header: HeaderSpectrum = HeaderSpectrum(name=name, 
                                                      category=category)
        self.signal: Optional[NamedSignal] = None

    def get_signal(self, recalculate=False, start_time: float = None,
            name: InName = None, category: str = None) -> 'NamedSignal':
        
        if self.signal is None or recalculate:
            signal = self._relation.get_signal(recalculate=recalculate, 
    ...
    def get_amp_spectrum(self, name: InName = None, 
                        category: str = 'amp_spectrum') -> NamedRelation:
    ...
    def get_phase_spectrum(self, name: InName = None,
                        category: str = 'phase_spectrum') -> NamedRelation:
    ...
    def get_reverse_filter(self: CSP, percent: Union[float, int] = 5, 
        ...
        spectrum = self._relation.get_reverse_filter(percent=percent, 
    ...
    def get_spectrum_from_amp_phase(
        ...
        header = HeaderSpectrum.get_spectrum_from_amp_phase(c_r1.header, 
...
class NamedSignal(NamedRelation):
    ...
    def get_amplitude_spectrum(self, recalculate=False, is_start_zero=False,
            name: InName = None, category: str = None) -> NamedRelation:
       
        amplitude_spectrum = self._relation.get_amplitude_spectrum(recalculate=recalculate, 
    ...
    def get_phase_spectrum(self, recalculate=False, is_start_zero=False,
            name: InName = None, category: str = None) -> NamedRelation:

        phase_spectrum = self._relation.get_phase_spectrum(recalculate=recalculate, 
                                            is_start_zero=is_start_zero)
        header = self.header.get_phase_spectrum(name, category)
    ...
    def get_spectrum(self, recalculate=False, is_start_zero=False,
            name: InName = None, category: str = None) -> NamedSpectrum:
        if self.spectrum is None or recalculate:
            spectrum = self._relation.get_spectrum(recalculate=recalculate, 
                                            is_start_zero=is_start_zero)

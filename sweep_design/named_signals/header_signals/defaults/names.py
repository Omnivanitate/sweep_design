...
def make_default_names_operations(obj1: HeaderBase, 
            obj2: Union[HeaderBase, Num], operation: MathOperation) -> str:
...
def make_default_interpolate_name(obj: HeaderBase, 
                                            new_x: np.ndarray) -> str:
    return 'inerp({0} {1}-{2} dx:{3})'.format(
            obj.name(), new_x[0], new_x[-1], new_x[1]-new_x[0]
            )

def make_default_name_correlation(obj1:HeaderBase, 
                                    obj2: HeaderBase) -> str:
    return '(Corr({0} & {1}))'.format(obj1._name(), obj2._name())

def make_default_name_convolution(obj1: HeaderBase, 
                                        obj2: HeaderBase) -> str:
...
def make_default_spectrum_from_amp_phase_name(obj_amp: HeaderBase, 
    obj_phase: HeaderBase) -> str:
    return 'SP(amp({0}) phase({1}))'.format(obj_amp, obj_phase)

def make_default_spectrum_from_phase_name( 
    obj_phase: HeaderBase) -> str:

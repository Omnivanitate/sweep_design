...
class NamedSweep(NamedSignal):
    ...
    def __init__(self, time: Any, amplitude: Any = None, name: InName = None, 
        ...
        self.aprior_signal: Optional[NamedSignal] = aprior_signal if aprior_signal is None else NamedSignal(aprior_signal)

        if isinstance(aprior_signal, Spectrum):
            self.aprior_signal = NamedSignal(aprior_signal.get_signal(), name=self.header)
    ...
    def spectrogram(self) -> 'NamedSpecrogram':
        if self._spectrogram is None:
            ...
            self._spectrogram = NamedSpecrogram(
            self._relation.spectrogram, name=header)

...
# from .config import get_figure_window, get_figure_frame, get_checkbox_frame, get_dropdown_frame

from ..framemakers.ipywidgetframer.figwidget.bokehfig import \
    default_line_property


def get_view_source_bokeh_ipywidget(reaction_mass = 1., limits = None, k_width = 1., k_height = 1.):
    ...
    figure_window = get_figure_window('Signals', 'Time, s', 'Force, N', k_width*0.8, k_height)
    figure_window.update({'extra_range_y_name': 'displacement', 'y_axis_label_2': 'Position, m'})
    figure_frame = get_figure_frame(k_width*0.8, k_height)
    checbox_frame = get_checkbox_frame('Sweep signals:', 'No sweep signals!', k_width*0.2, k_height)

...
from ..base_view.abc_common_framer_grapher.frame import CommonCheckBoxFrame, CommonDropdownFrame, CommonFigureFrame, CommonCheckBox
...
class InteracterSource(CommonController):
    ...
    def _call_checkbox_types_signals(self, group: str, value: bool):
        ...
        for line in self._signal_figure_frame.get_figure().get_lines_by_groups(set([group, current_dropdown_choise])):
            line.set_visible(value['new'])
        if group == 'limits':
            for line in self._signal_figure_frame.get_figure().get_lines_by_groups(set([group])):
                line.set_visible(value['new'])
    ...
    def _call_dropdown_signal(self, value: Dict[str, Any]):
        value_new = value['new']
        for line in self._signal_figure_frame.get_figure().get_lines_by_groups(set([value_new])):
            line.set_visible(True)
        value_old = value['old']
        for line in self._signal_figure_frame.get_figure().get_lines_by_groups(set([value_old])):

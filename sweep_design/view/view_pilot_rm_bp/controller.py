...
from ..base_view.abc_common_framer_grapher.frame import (
    CommonCheckBoxFrame, CommonDropdownFrame, CommonFigureFrame)
...
class InteracterPilotRmBp:
    ...
    def _call_checkbox_signal(self, checkbox_name: str, value: bool) -> None:
        for lines in self._amp_spectrum_figure_frame.get_figure().get_lines_by_groups(checkbox_name):
            lines.set_visible(value)
        
        for lines in self._phase_spectrum_figure_frame.get_figure().get_lines_by_groups(checkbox_name):
            lines.set_visible(value)
        
        for lines in self._signal_figure_frame.get_figure().get_lines_by_groups(checkbox_name):
            lines.set_visible(value)
        
        for lines in self._spectogram_figura_frame.get_figure().get_lines_by_groups(checkbox_name):
            lines.set_visible(value)
        
        for images in self._spectogram_figura_frame.get_figure().get_images_by_groups(checkbox_name):

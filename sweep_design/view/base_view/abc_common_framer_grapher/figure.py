...
from typing import Any, Dict, Iterable, List, Literal, Optional, Set, Union, TypeVar, Generic
...
class CommonFigure(Generic[Figure], metaclass=ABCMeta):
    ...
    @abstractmethod
    def add_line(self, x: np.ndarray, y: np.ndarray, unique_name: str, 
        groups: Union[str, Set[str]], line_properties: Dict[str, Any]) -> 'CommonLine':
        pass
    
    @abstractmethod
    def add_image(self, x: np.ndarray, y: np.ndarray, s_xy: np.ndarray, unique_name: str,
        groups: Set[str], image_properties: Dict[str, Any]) -> None:
        pass
    
    @abstractmethod
    def add_infty_line(self, k: float, axis: Literal['x', 'y'], unique_name: str, 
    ...
    def get_lines_by_groups(self, groups: Union[str, Set[str]]) -> List['CommonLine']:
        if isinstance(groups, str):
            groups = set([groups])
        return [line for line in self._lines.values() if groups.issubset(line.get_groups())]

    def get_images_by_groups(self, groups: Union[str, Set[str]]) -> List['CommonImage']:
        if isinstance(groups, str):
            groups = set([groups])
        return [image for image in self._images.values() if groups.issubset(image.get_groups())]

...
class GeneralView:
    ...
    def __init__(
        ...
        self._checkbox = framer.get_checkbox_frame(checkbox_properties, self._controller)
        self._dropdown = framer.get_dropdown_frame(dropdown_properties, self._controller)
    ...
    def add_line(self, x: Union[NamedRelation,  np.ndarray], y: np.ndarray = None, name: str = None, line_properties: Dict[str, Any] = None):
        ...
        if isinstance(x, (Relation, NamedRelation)):
            if name is None: 
                name = str(x)
            line = self._frame.get_figure().add_line(*x.get_data(), name, None, line_properties)

        else: 
            line = self._frame.get_figure().add_line(x, y, name, None, line_properties)
        
        self._checkbox.add_choise(name, None, {'color':line.get_color()}, self._controller)

    def add_image(self, x, y: np.ndarray = None, s_xy: np.ndarray = None, name: str = None, image_properties: Dict[str, Any] = None):
        ...
        if isinstance(x, (NamedSweep, NamedSpecrogram)):
            ...
            if isinstance(x, NamedSweep):
                ...
                spectrogram = x
            self._frame.get_figure().add_image(spectrogram.time, spectrogram.frequency, spectrogram.spectrogram_matrix, unique_name=name, groups=None, image_properties=image_properties)

...
class CommonVeiwSweepBuilder:
    ...
    def __init__(self,
        ...
        self._signal_frame = framer.get_figure_frame(signal_figure, \
            properties_signal_frame)
        ...
        self._spectrum_amplitude_frame = framer.get_figure_frame(spectrum_amplitude_figure, properties_freq_amplitude_frame)
        ...
        self._sweep_frame = framer.get_figure_frame(sweep_figure, \
            properties_sweep_frame)
        ...
        spectrum_section = self._spectrum_amplitude_frame.h_add(self._spectrum_phase_frame)
        result_frame = self._signal_frame.v_add(spectrum_section)
        result_frame = result_frame.h_add(self._checkbox_signals_frame)
        sweep_section = self._sweep_frame.v_add(self._dropdown_images_spectrogram).v_add(self._spectrogram_images_frame).v_add(self._envelop_frame).h_add(self._checkbox_sweep_frame)
    ...
    def add(self, data: Union[NamedSignal, NamedSweep]):
        ...
        if isinstance(data, NamedSweep):
            self._sweep_frame.get_figure().add_line(*data.get_data(), name, None, line_properties)
            
            spectrogram_figure = self._spectrogram_images_frame.get_figure()
            
            s = data.spectrogram
            spectrogram_figure.add_image(s.time, s.frequency, s.spectrogram_matrix, name, None, image_properties)
            spectrogram_figure.add_line(*data.f_t.get_data(), name, None, line_properties)
            self._dropdown_images_spectrogram.add_choise(name)

            self._envelop_frame.get_figure().add_line(*data.a_t.get_data(), name, None, line_properties)
            self._checkbox_sweep_frame.add_choise(name, None, checkbox_properties, self._controller)

            
        elif isinstance(data, NamedSignal):
            
            self._signal_frame.get_figure().add_line(*data.get_data(), name, None, line_properties)
            self._checkbox_signals_frame.add_choise(name, None, checkbox_properties, self._controller)
        ...
        self._spectrum_amplitude_frame.get_figure().add_line(
            *amplitude_spectrum.get_data(), str(amplitude_spectrum), None, line_properties 
            )

...
class BokehFigureMaker(CommonFigureMaker):
    def __init__(self, default_line_property=default_line_property, 
...
class BokehFigure(widgets.Output, CommonFigure[Figure], metaclass=MetaFigure):
    ...
    def add_extra_axis(self, axis_name: str, axis_label: str, location='right') -> None:
        ...
        self._figure.add_layout(LinearAxis(y_range_name = axis_name, axis_label = axis_label), location)
    ...
    def set_limits(self, range_axis: DataRange1d, min_z=0, max_z=1,):
        
        range_axis.start = min_z + min_z*0.25
        range_axis.end = max_z + max_z*0.25   
        
    def add_line(self, x: np.ndarray, y: np.ndarray, name: str, groups: Set[str], line_property: Dict[str, Any]) -> 'BokehLine':
    ...
    def add_image(self, x: np.ndarray, y: np.ndarray, s_xy: np.ndarray, unique_name: str, groups: Union[str, Set[str]], image_properties: Dict[str, Any]) -> None:
        
        if isinstance(groups, str):
            groups = set([groups])

        image = self._figure.image(image=[s_xy], x=x[0], y=y[0],
            dw = x[-1] - x[0], dh = y[-1] - y[0], **image_properties)
        image.level = 'underlay'
        self._images.update({unique_name: BokehImage(image, unique_name, groups, self.update)})
   
    def add_infty_line(self, k: np.ndarray, axis: Literal['x', 'y'], unique_name: str, 
...
class BokehLine(CommonLine[GlyphRenderer]):
    ...
    def __init__(self, line: GlyphRenderer, name: str, groups: Union[str, Set[str]], update: Any) -> None:
...
class BokehImage(CommonImage[GlyphRenderer]):
    ...
    def __init__(self, image: GlyphRenderer, name: str, groups: Set[str], update: Callable[..., None]) -> None:

...
class MatplotlibFigureMaker(CommonFigureMaker):
    ...
    def __init__(self, default_line_properties = _default_line_properties,
        color_palette= _color_palette) -> None:
    ...
    def get_figure(figure_properties: Dict[str, Any]) -> 'PltFigure':
        fig, axes = plt.subplots(
            ...
            figsize=(0.75*figure_properties['width']*_px, 
...
class PltFigure(widgets.Output, CommonFigure[Figure], metaclass=MetaFigure):
    ...
    def add_line(self, x, y, unique_name: str, groups: Union[str, Set[str]], line_property: dict) -> 'PltLine':
    ...
    def add_image(self, x: np.ndarray, y: np.ndarray, s_xy: np.ndarray, unique_name: str, groups: Set[str],
        image_properties: dict) -> None:
        extent = (x[0], x[-1], y[0], y[-1])
        image = self.axes.imshow(s_xy, extent=extent, origin = 'lower', aspect='auto', cmap = image_properties)
        self._images.update({unique_name: PltImage(image, unique_name, groups, self.update)})

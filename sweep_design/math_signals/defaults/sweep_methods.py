...
def pre_interpolate_time(x: Union[RelationProtocol, np.ndarray], y: np.ndarray = None) \
        -> InterpolateTime:
...
def convert_freq2time(spectrum: 'Spectrum', convert_method: CallFtatMethod) \
...
def get_info_from_aprior_data(t: Any, aprior_data: Any,
        f_a_t_method: CallFtatMethod) -> Tuple[InterpolateTime, InterpolateTime, 'Signal']:
...
def _extract_x_t(t: Optional[np.ndarray], x_t: Ftatr) -> Tuple[Optional[np.ndarray], InterpolateTime]:
        ...
        if isinstance(x_t, Relation):
            ...
            if isinstance(t, np.ndarray) and isinstance(x_t, np.ndarray):
                if t.size != x_t.size:
                    ...
                    x_t = Config.interpolate_extrapolate_method(
                        calc_t, x_t)
                    x_t = x_t(t)
...
def get_spectrogram(nperseg=256, noverlap=128, nfft=2048) \
        -> Callable[[np.ndarray, np.ndarray], Tuple[time, frequency, spectrogram_]]:
    '''Function to prepare a function that retrieves the spectrogram of the sweep signal.

    '''
    def extract(time: np.ndarray, amplitude: np.ndarray) \
        ...
        dt = time[1]-time[0]
        frequency, spectrogram_time, spectrogram_ = spectrogram(amplitude, 1/(dt), nperseg=nperseg,
...
def get_f_t(time: np.ndarray, amplitude: np.ndarray) -> Relation:
    ...
    result = np.append([0.], np.diff(np.unwrap(np.angle(analytical_signal)))/(2.0*np.pi)/(time[1]-time[0]))
...
def integrate_quad(f_t_function: Callable[[time], frequency], time: np.ndarray) -> theta:
    '''Integration.

...
def signal2spectrum(time: np.ndarray, amplitude: np.ndarray, 
                        is_start_zero=False) -> Tuple[frequency, spectrum]:
    ...
    if not is_start_zero:
        if time[0] > 0.:
            time = np.linspace(0., time[-1], int(time[-1]/(time[1]-time[0]))+1)
            amplitude = np.append(np.zeros(time.size-amplitude.size), amplitude)
        elif time[-1] < 0.:
            time = np.linspace(time[0], 0., int(abs(time[0])/(time[1]-time[0]))+1)
            amplitude = np.append(amplitude, np.zeros(time.size-amplitude.size))
        
        amplitude = np.append(amplitude[time>=0.], amplitude[time<0.])
    
    spectrum = np.fft.rfft(amplitude)
    frequency = np.fft.rfftfreq(amplitude.size, d=(time[-1]-time[0])/(amplitude.size))
    return frequency, spectrum


def spectrum2sigmal(frequency: np.ndarray, spectrum: np.ndarray, 
                    time_start: float = None) -> Tuple[time, amplitude]:
    ...
    if time_start is None:
        time = np.linspace(0, (amplitude.size-1)/(2*(frequency[-1]-frequency[0])), amplitude.size)
    else:
        time = np.linspace(time_start, time_start+(amplitude.size-1)/(2*(frequency[-1]-frequency[0])), amplitude.size)
        amplitude = np.append(amplitude[time>=0.], amplitude[time<0.])

...
def tukey_a_t(time: np.ndarray, t_tapper: float, location: Literal['left', 'right', 'both'] = 'both'):
    ...
    result = np.append(result[:int(time.size/2)], np.ones(time.size-int(time.size/2)))

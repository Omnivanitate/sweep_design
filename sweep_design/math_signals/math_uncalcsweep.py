...
class UncalculatedSweep:
    ...
    **Properties**:
    ...
    def __init__(self, time: NDArray = None, frequency_time: Union[Ftatr, NDArray] = None, 
                                    amplitude_time: Union[Ftatr, NDArray] = None) -> None:
        
        self._integrate_function_default = SweepConfig.integrate_function
        
        time = time if time is None else np.array(time)
        if not (isinstance(frequency_time, (np.ndarray, Relation)) or callable(frequency_time) or frequency_time is None):
            frequency_time = np.array(frequency_time)
        
        if not (isinstance(amplitude_time, (np.ndarray, Relation)) or callable(amplitude_time) or amplitude_time is None):
            amplitude_time = np.array(amplitude_time)


        self._time, self._frequency_time, self._amplitude_time \
    ...
    def __call__(self, time: NDArray = None, tht0=0.) -> Sweep:
        ...
        return Sweep(time=calc_time, amplitude=sweep, frequency_time=frequency_time, amplitude_time=amplitude_time)


    def _func_tht(self, frequency_time: Callable[[np.ndarray], np.ndarray]) \
    ...
    def _array_tht(self, frequency_time: np.ndarray) -> Callable[[np.ndarray], np.ndarray]:
        '''Angular sweep represented by a numerical sequence.'''
        def result(time: np.ndarray) -> np.ndarray: 
            frequency_time_relation = Relation(time, frequency_time)
            return np.append(
            [0.], 2*np.pi*frequency_time_relation.integrate().y)
        return result
...
class ApriorUncalculatedSweep(UncalculatedSweep):
    ...
    def __init__(self, time: Any = None, aprior_data: Any = None, 
                 ftat_method: CallFtatMethod = None) -> None:
    
        if not ftat_method:
            ftat_method = SweepConfig.freq2time

        frequency_time, amplitude_time, self._aprior_signal \

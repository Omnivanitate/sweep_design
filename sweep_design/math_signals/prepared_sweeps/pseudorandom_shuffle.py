...
def get_shuffle(
    ...
    if dt >= length_time_segments:
        raise SegmentSmallerDtError(f'Length of shuffle time segment \
    ...
    if not round_number_freqency: 
        ...
        f_segment = np.round(np.linspace(f_start, f_end, n_segments+1), round_number_freqency)

    random.shuffle(f_segment)

    x = np.linspace(0., length_time_segments, math.ceil(length_time_segments/dt)+1)

...
of varying complexity. It contains not only tools for creation, but also tools for analysis, 
visualization and other purposes. It can be used both for educational and production purposes.  
...
For example, write your own methods describing how the frequency and amplitude 
will change from the time of the sweep signal.  
...
> You can install the library using `pip install sweep_design`, or you can clone or 
load it from GitHub, add path to the module and install requremnt 
...
* `dataio` - contains simple functions for loading and saving data. For a difficult 
situation, write your own.
* `math_signals` - There is the core of project. Here is the basic concept of creating signals, 
specectra and sweep-signals realising. 
* `named_signals` - There is a representation of the created math_signals object. 
For simplicity, each represented object from named_signals has an additional field like name and category. 
This is convenient when you work with many signals, spectra and you don't to get confused.
* `view` - There is a sub-module for visualasing results. Contains a few GUI for jupyter notebook.

- - -
For convenient you can directly import a few objects from modules `sweep_design`, such as 
```python
...
from .math_signals import utility_functions as utility_functions 